...
import { OpenRouterClient, PipelineContext, _buildStatReport, _buildComparisonChartArtifact, _buildBaselineChartArtifact } from './ai/openrouter_client';
...
// Python resolver
// ─────────────────────────────────────────────────────────────────────────────
let _cachedPyCmd: string | null = null;
function resolvePythonCommand(): string {
    ...
    if (_cachedPyCmd) { return _cachedPyCmd; }
...
function _normaliseLeakageForDocx(lk: any): Record<string, any> {
    if (!lk) { return {}; }
    return {
...
function createToolExecutor(
    ...
    getState: () => { context: PipelineContext; baseline: any; filePath: string | null },
    setState: (patch: Partial<PipelineContext>) => void,
    getUserMessage: () => string,
): { executor: import('./ai/openrouter_client').ToolExecutor; getReportArtifact: () => { type: 'report'; content: string; filePath: string | null; reportType: string; chart?: any } | null } {
    ...
    let _pendingReportArtifact: { type: 'report'; content: string; filePath: string | null; reportType: string; chart?: any } | null = null;

    const executor = async (name: string, args: Record<string, any>): Promise<string> => {
        ...
        switch (name) {
            ...
            case 'get_dataset_state': {
                ...
                const snap = {
                    ...
                    column_stats: Object.fromEntries(
                        Object.entries(ctx.baseline?.columns?.numeric ?? {}).map(
                            ([col, stats]: [string, any]) => [
                                col,
                                {
                                    mean:   stats.mean   != null ? parseFloat((stats.mean   as number).toFixed(4)) : null,
                                    std:    stats.std    != null ? parseFloat((stats.std    as number).toFixed(4)) : null,
                                    min:    stats.min    ?? null,
                                    max:    stats.max    ?? null,
            ...
            case 'generate_rows': {
                ...
                const count: number = typeof args.count === 'number'
            ...
            case 'merge_and_update': {
                ...
                const newRows: any[] = _pendingRows.length > 0
                    ? _pendingRows
                ...
                const existing: any[] = current.result?.samples ?? [];
            ...
            case 'edit_dataset': {
                const { context: current } = getState();
                let samples: any[] = [...(current.result?.samples ?? [])];
                if (samples.length === 0 && current.baseline?.preview_rows) {
                    samples = [...current.baseline.preview_rows];
                }
                const action: string = args.action ?? 'delete_rows';
                const colName: string = args.column_name ?? '';
            ...
            case 'analyze_dataset': {
                ...
                const focus: string = args.focus ?? 'full';
                ...
                const allRows: any[]   = cur.result?.samples ?? [];
                ...
                if (allRows.length === 0 && (focus === 'statistical' || focus === 'full')) {
                    ...
                    const rawFocus: string = focus;
                    const baselineStats: Record<string, any> = {};
                    for (const [col, stats] of Object.entries(cols.numeric ?? {}) as [string, any][]) {
                        baselineStats[col] = {
                            min:    stats.min    ?? null,
                            max:    stats.max    ?? null,
                            mean:   stats.mean   != null ? parseFloat((stats.mean  as number).toFixed(4)) : null,
                    ...
                    if (Object.keys(baselineStats).length > 0) {
                        const privacyFallback = (rawFocus === 'privacy' || rawFocus === 'full') ? {
                            ...
                            attack_report: (cur as any).attackReport  ?? null,
                        } : null;
                        const driftFallback = (rawFocus === 'drift' || rawFocus === 'full') ? {
                            drift_scores:   (cur.leakage as any)?.drift_scores ?? (cur.leakage as any)?.column_drift ?? null,
                ...
                const statisticalAnalysis: Record<string, any> = {};

                if (focus === 'statistical' || focus === 'full') {
                    for (const col of numericCols) {
                        const vals: number[] = allRows
                ...
                return JSON.stringify({
                    ...
                    baseline: {
                        ...
                        statistical: (() => {
                            ...
                            for (const [col, st] of Object.entries(cols.numeric ?? {}) as [string, any][]) {
                                bs[col] = {
                                    min:    st.min    ?? null,
                                    max:    st.max    ?? null,
                                    mean:   st.mean   != null ? parseFloat((st.mean  as number).toFixed(4)) : null,
                                    std:    st.std    != null ? parseFloat((st.std   as number).toFixed(4)) : null,
            ...
            case 'stat_summary': {
                ...
                let scope: 'baseline' | 'synthetic' | 'comparison' =
                    msgBaseline  ? 'baseline'    :
                    msgSynthetic ? 'synthetic'   :
                    msgCompare   ? 'comparison'  :
                    (args.scope as 'baseline' | 'synthetic' | 'comparison' | undefined) ?? 'comparison';
                ...
                try {
                    ...
                    const contextForReport: PipelineContext = {
                        ...lastPipelineContext,
                        baseline: lastPipelineContext.baseline ?? lastBaseline,
                    };
                    // Build full report without any LLM call
                    const content: string = _buildStatReport(contextForReport, scope);

                    // Build JSON payload for aurora_stat_docx.py
                    const b_cols: Record<string, any> = lastBaseline?.columns?.numeric ?? {};
                    const samples: any[] = lastPipelineContext.result?.samples ?? [];
                    // Compute synthetic stats (max 1000 rows)
                    const synStats: Record<string, any> = {};
                    const slice = samples.slice(0, 1000);
                    if (slice.length > 0) {
                        for (const col of Object.keys(b_cols)) {
                            const vals = slice.map((r: any) => Number(r[col])).filter((v: number) => !isNaN(v));
                            if (vals.length === 0) { continue; }
                            const mean = vals.reduce((a: number, b: number) => a + b, 0) / vals.length;
                            const sorted = [...vals].sort((a, b) => a - b);
                            const std = Math.sqrt(vals.reduce((a: number, b: number) => a + Math.pow(b - mean, 2), 0) / vals.length);
                    ...
                    try {
                        ...
                        } catch (e2) { console.error('[Aurora] fallback DOCX also failed:', e2); }
                    }
                    ...
                    const baselineChartArtifact = (scope === 'baseline')
                        ? _buildBaselineChartArtifact(contextForReport.baseline ?? lastBaseline)
                        : null;
                    _pendingReportArtifact = {
                        type: 'report',
                        content,
            ...
            case 'generate_report': {
                const { context: cur } = getState();
                const reportType: string = args.type ?? 'full';
                try {
                    let fullContent = '';
                    if (filePath && baseline) {
                    ...
                    let tmpDocxPath: string | null = null;
                    if (fullContent) {
                        try {
                            ...
                            try { fs.unlinkSync(mdTmp2); } catch { }
                            tmpDocxPath = fs.existsSync(tmpDocx) ? tmpDocx : null;
                        } catch (e) { console.error('[Aurora] generate_report DOCX failed:', e); }
                    }
            ...
            case 'export_csv': {
                const { context: cur } = getState();
                const rows: any[] = cur.result?.samples ?? [];
                if (rows.length === 0) {
                    return JSON.stringify({
                        error: 'No synthetic data in memory. Run the generator first.',
                    });
                }
                const filename: string = (args.filename as string) || 'aurora_dataset.csv';
            ...
            case 'combine_datasets': {
                const { context: cur } = getState();
                const syntheticRows: any[] = cur.result?.samples ?? [];
                ...
                try {
                    ...
                    const originalRows: any[] = csvLines.slice(1).map((line: string) => {
                    ...
                    const taggedOriginal  = originalRows.map((r: any) => ({ ...r, _source: 'original' }));
                    const taggedSynthetic = syntheticRows.map((r: any) => ({ ...r, _source: 'synthetic' }));
        ...
        vscode.commands.registerCommand("idelense.parseDataset", async (lineText: string) => {
            ...
            try {
                ...
                showCheckpointMonitor(
                    ...
                    null, ast, baseline, null, null, null, null
                );
            } catch (err: any) {
...
function runGenerator(context: vscode.ExtensionContext, filePath: string, baselinePath: string, n: number, forceEngine: string = 'auto'): Promise<any> {
    return new Promise(async (resolve, reject) => {
        const py = resolvePythonCommand();
...
let lastReportArtifact: { type: 'report'; content: string; filePath: string | null; reportType: string } | null = null;
...
function runStatDocx(
    ...
    return new Promise(async (resolve, reject) => {
        ...
        const { stderr, code } = await collectOutput(
            spawnPython(py, [scriptPath, dataJsonPath, docxPath])
        );
        ...
        if (fs.existsSync(docxPath)) { resolve(); return; }
        if (code !== 0) { reject(new Error(stderr || `aurora_stat_docx.py exited ${code}`)); return; }
        resolve();
    });
}
    ...
    panel.webview.onDidReceiveMessage(async (msg: any) => {
        try {
            ...
            if (msg.command === 'exportCSV') {
                const fmt: string = (msg.format as string) || 'csv';
                const defaultName: string = (msg.filename as string) ?? `synthetic_data.${fmt}`;
                ...
                if (choice === 'Open in Editor') { vscode.window.showTextDocument(uri); }
            }
            if (msg.command === 'exportReport') {
                const defaultName: string = (msg.filename as string) ?? 'report.json';
                const uri = await vscode.window.showSaveDialog({
                    defaultUri: vscode.Uri.file(path.join(
                        vscode.workspace.workspaceFolders?.[0]?.uri.fsPath ?? os.homedir(),
                        defaultName
            ...
            if (msg.command === 'exportArtifact') {
                try {
                    const prebuiltPath: string | null = (msg.filePath && fs.existsSync(String(msg.filePath)))
                        ? String(msg.filePath) : null;
                    const defaultName: string = prebuiltPath
                        ? path.basename(prebuiltPath)
                        : ((msg.filename as string) ?? 'aurora_report.docx');
            ...
            if (msg.command === 'agentChat') {
                ...
                try {
                    ...
                    const { executor: toolExecutor, getReportArtifact } = createToolExecutor(
                        context,
                        panel,
                        () => ({
                    ...
                    const onToolProgress = (toolName: string, args: Record<string, any>) => {
                        panel.webview.postMessage({
                    ...
                    let _lastAnalysisResult: string | null = null;
                    const wrappedToolExecutor = async (name: string, args: Record<string, any>) => {
                        const result = await toolExecutor(name, args);
                        if (name === 'analyze_dataset') { _lastAnalysisResult = result; }
                        return result;
                    };
                    ...
                    if (!( response as any).artifact && _lastAnalysisResult) {
                        const chartArtifact = _buildComparisonChartArtifact(_lastAnalysisResult);
                        if (chartArtifact) { (response as any).artifact = chartArtifact; }
                    }
                    ...
                    const isGenRequest = !isStatOrReportRequest && (
                        (
                            /\b(generat|add|creat|mak|produc)\w*\b/i.test(msg.message ?? '') &&
                            /\b(rows?|records?|samples?)\b/i.test(msg.message ?? '')
                        ) || /\b(combin|merg)\w*\b/i.test(msg.message ?? '')
                    );
                    if (isGenRequest) {
                        ...
                        (response as any).artifact = { type: 'csv', filePath: null };
                    }
                    ...
                    if (pendingReport) { lastReportArtifact = pendingReport; }
                    if (lastReportArtifact && !(response as any).artifact) {
                        (response as any).artifact = lastReportArtifact;
                    }
                    ...
                    panel.webview.postMessage({
                        ...
                        artifact: (response as any).artifact ?? null,
                        msgId:    msg.msgId,
                    });
                } catch (err: any) {
            ...
            if (msg.command === 'agentStatReport') {
                try {
                    ...
                    const fullContent: string = _buildStatReport(lastPipelineContext, scope);
                    ...
                    try {
                        ...
                        } catch (e2) { console.error('[Aurora] agentStatReport fallback also failed:', e2); }
                    }
