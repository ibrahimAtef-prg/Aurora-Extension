...
export function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('aurora.switchTheme', async () => {
            ...
            if (!choice) { return; }
            const theme = choice.value as 'aurora' | 'dark';
...
// ─────────────────────────────────────────────────────────────────────────────
function buildReportStyles(theme: 'aurora' | 'dark'): string {
    if (theme === 'aurora') {
        return `<style>
...
function showScanReport(context: vscode.ExtensionContext, report: any, filePath: string): void {
    ...
    const findingsHtml = [...(report.pii_findings || []), ...(report.secrets || []), ...(report.sensitive_content || [])]
        ...
        .join('');
    const theme = context.globalState.get<'aurora'|'dark'>('aurora.theme', 'aurora');
...
function showCheckpointMonitor(
    ...
    const savedTheme = context.globalState.get<'aurora'|'dark'>('aurora.theme', 'aurora');
    panel.webview.html = buildMonitorHtml(dashboardData, savedTheme);
    monitorPanel = panel;
