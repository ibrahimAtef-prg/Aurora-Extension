...
class RegexDetector:
    ...
    PII_PATTERNS = {
        ...
        "ssn": {
            "pattern": r'\b(?!000|666)[0-8]\d{2}[- ]?(?!00)\d{2}[- ]?(?!0000)\d{4}\b',
            "severity": "critical",
    ...
    def scan_value(self, value: str, column: str, row_idx: int) -> List[Finding]:
        ...
        for cat, rx in self._compiled_pii.items():
            # SSN: only scan columns explicitly named for SSNs
            if cat == "ssn" and column.lower() not in ("ssn", "social_security", "social_security_number"):
                continue
            # Passport: only scan columns explicitly named for passports
            if cat == "passport" and "passport" not in column.lower():
                continue
            if rx.search(value):
...
class DataScanner:
    ...
    def scan_dataframe(self, df, max_rows: Optional[int] = None) -> ScanReport:
        ...
        report = ScanReport()
        report.columns_scanned = sample.shape[1]
        report.rows_scanned = sample.shape[0]
        ...
        for col in df.columns:
            for row_idx, val in sample[col].items():
                ...
                if len(val) < 3:
                    continue
                all_findings.extend(self.regex_detector.scan_value(val, col, row_idx))
                all_findings.extend(self.entropy_detector.scan_value(val, col, row_idx))
        ...
        report.summary = (
            f"Scanned {report.rows_scanned} rows ({report.total_cells_scanned} cells) "
            f"across {report.columns_scanned} columns. "
