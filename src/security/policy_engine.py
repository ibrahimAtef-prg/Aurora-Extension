...
class PolicyEngine:
    ...
    def __init__(self, policy_path: Optional[str] = None):
        ...
        if policy_path and os.path.exists(policy_path):
            try:
                ...
                # YAML failed — store error, fall back to default policy thresholds
                self._policy_error = str(e)
                self.rules = list(BUILTIN_RULES.keys())
                self.thresholds = {"max_risk_score": 50}
                self.default_action = "warn"
        else:
    ...
    def evaluate(self, scan_report) -> PolicyResult:
        ...
        max_risk = self.thresholds.get("max_risk_score", 50)
