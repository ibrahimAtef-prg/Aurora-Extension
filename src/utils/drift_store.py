            ...
            try:
                ...
                if pd.api.types.is_numeric_dtype(series):
                    ...
                    if len(vc) > _CATEGORICAL_CARDINALITY_CAP:
                        ...
                        continue
                    # Secondary guard: near-unique columns (IDs, emails on
                    # small datasets) have disjoint value sets every run →
                    # guaranteed JS ≈ log(2) false positive.
                    if len(vc) > 0 and (len(vc) / max(len(series), 1)) > 0.8:
                        continue
                    col_stats[col] = {
                        "type": "categorical",
