...
def _extract_column_drift(synthetic_df, original_df) -> Dict[str, float]:
    ...
    # ── Full-pipeline numeric drift (if the governance core is present) ──
    try:
        ...
        for col, v in per_column.items():
            result[col] = round(float(v.get("shift_score", 0.0)), 4)
    except Exception as exc:
        print(f"[leakage_bridge] numeric column_drift extraction failed: {exc}", file=sys.stderr)

    # ── Categorical drift fallback via JS divergence ─────────────────────
    # Covers columns the pipeline skipped (categorical or absent from per_column).
    cat_cols = [
        c for c in original_df.columns
        if c not in result and not pd.api.types.is_numeric_dtype(original_df[c])
    ]
    for col in cat_cols:
        try:
            orig_vc = original_df[col].dropna().astype(str).value_counts(normalize=True)
...
def main(argv=None):
    ...
    def _emit_error(msg: str) -> int:
        ...
        _sys.stdout.write(json.dumps(out, ensure_ascii=False) + "\n")
        return 1
    ...
    if pipeline_root is not None:
        if pipeline_root not in _sys.path:
            _sys.path.insert(0, pipeline_root)
        try:
            ...
            try:
                ...
                try:
                    ...
                    pass
                _sys.stdout.write(json.dumps(output, ensure_ascii=False, default=str) + "\n")
                return 0
            except Exception as exc:
                traceback.print_exc(file=_sys.stderr)
                _sys.stderr.write(f"[leakage_bridge] Full pipeline failed ({exc}); falling back to local mode.\n")
        except ImportError as exc:
            _sys.stderr.write(f"[leakage_bridge] Pipeline import failed ({exc}); using local mode.\n")
