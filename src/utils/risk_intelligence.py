...
def rank_sensitive_columns(
    ...
    if len(num_df.columns) > 1:
        ...
        for col in num_df.columns:
            ...
            val = others.max()
            # others.max() returns NaN for all-NaN slices (zero-variance columns).
            # float('nan') serialises to the bare token NaN in Python's json module,
            # which is not valid JSON and causes JSON.parse to throw in extension.ts.
            corr_map[col] = float(val) if (len(others) and val == val) else 0.0
