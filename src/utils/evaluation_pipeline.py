...
def evaluate(
    ...
    # ── 5. Quick TSTR R² sanity check ─────────────────────────────────────
    tstr_r2: Optional[float] = None
    if orig_df is not None:
        target_col = baseline.get("meta", {}).get("label_col") or _infer_numeric_target(orig_df)
        tstr_r2 = _quick_tstr_r2(orig_df, synth_df, target_col)
        if tstr_r2 is not None and tstr_r2 < 0.0:
            warnings_out.append(
                f"TSTR R² = {tstr_r2:.4f}: model trained on synthetic data predicts "
                "real data worse than the mean baseline — joint structure may be broken."
            )
...
def _infer_numeric_target(df: pd.DataFrame) -> Optional[str]:
    """Heuristic: pick the numeric column most likely to be a regression target."""
    num_cols = [c for c in df.columns if pd.api.types.is_numeric_dtype(df[c])]
    if not num_cols:
        return None
    # Prefer columns whose name suggests a target (charges, price, salary, etc.)
    _TARGET_HINTS = ("charge", "price", "salary", "cost", "income", "revenue",
                     "target", "label", "score", "value", "amount")
    for col in num_cols:
        if any(h in col.lower() for h in _TARGET_HINTS):
            return col
    return num_cols[-1]  # fallback: last numeric column
...
def _quick_tstr_r2(
    ...
    """Train-on-Synthetic, Test-on-Real R² via Ridge regression.

    Returns None when the check cannot run (too few features, sklearn absent,
    etc.).  Returns a float in (-∞, 1] otherwise; values below 0 indicate the
    model is worse than predicting the mean, which signals broken joint structure.
    """
    ...
    try:
        ...
        feature_cols = [c for c in real_df.columns
                        if c != target_col and pd.api.types.is_numeric_dtype(real_df[c])]
        if len(feature_cols) < 2:
            return None
        X_s = synth_df[feature_cols].fillna(0).values
        y_s = synth_df[target_col].fillna(0).values
        X_r = real_df[feature_cols].fillna(0).values
        y_r = real_df[target_col].fillna(0).values
        sc = StandardScaler()
        model = Ridge(alpha=1.0)
        model.fit(sc.fit_transform(X_s), y_s)
        return round(float(model.score(sc.transform(X_r), y_r)), 4)
    except Exception:
        return None
