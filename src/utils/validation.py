...
class ConstraintFilter:
    ...
    def apply(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, int]:
        ...
        # Numeric columns that the engine skipped entirely must be populated with
        # samples from their marginal distribution — NOT a single constant.
        # A constant fill (e.g. all 500 rows = q50) triggers NUMERIC_COLLAPSE.
        # We use the same piecewise-linear quantile CDF already defined in this
        # module to produce varied values that respect the baseline shape.
        for col in bl.col_order:
            if col not in df.columns:
                if col in bl.numeric:
                    spec   = bl.numeric.get(col, {})
                    n_rows = len(df)
                    ...
                    if (
                        levels is not None
                        and len(values) >= 2
                        and float(values[-1]) > float(values[0])  # non-degenerate range
                    ):
                        u = rng.uniform(0.0, 1.0, size=n_rows)
                        df[col] = np.interp(u, levels, values)
                        filled = True
                    if not filled:
                        # Degenerate spec — constant fallback (column is truly invariant)
                        fallback = (
                            spec.get("q50") or spec.get("mean") or
                            spec.get("min") or 0.0
                        )
                        try:
                            fill_val = float(fallback) if fallback is not None else 0.0
                        except (TypeError, ValueError):
                            fill_val = 0.0
                        if fill_val != fill_val:  # isnan guard
                            fill_val = 0.0
                        df[col] = fill_val
                else:
                    df[col] = None
