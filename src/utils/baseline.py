...
class CategoricalColumnBaseline:
    ...
    # Full distribution (all values) stored for low-cardinality columns so
    # StatisticalEngine can sample from the complete distribution instead of
    # the truncated top-k. None for high-cardinality columns.
    all_value_ratios: Optional[Dict[str, float]] = field(default=None)
    ...
    *,
    ...
    for col in numeric_cols:
        ...
        if len(non_null):
            ...
            # Detect integer-only columns: dtype is integer, OR all non-null
            # float values have zero fractional part (e.g. Age=25.0, Year=2023.0).
            orig_dtype = str(df[col].dtype)
            if orig_dtype.startswith("int"):
                nb.is_integer = True
            else:
                # Float column: check whether every value is a whole number.
                # Use a random sample (not head()) to avoid bias from
                # sorted/sequential data where the first 2000 rows may all be
                # integers even if later rows are fractional (G6 fix).
                sample_check = non_null.sample(min(2000, len(non_null)), random_state=0)
                nb.is_integer = bool((sample_check % 1 == 0).all())
    ...
    for col in categorical_cols:
        ...
        # Build full value distribution for low-cardinality columns (G3 fix).
        # This lets StatisticalEngine sample from the complete distribution
        # instead of being limited to the top-k values.
        all_ratios: Optional[Dict[str, float]] = None
        if unique_count <= max_allowed_values_store:
            denom_all = int(s_non_null.shape[0]) if int(s_non_null.shape[0]) else 1
            all_ratios = {str(k): (int(v) / denom_all) for k, v in vc.items()}
    ...
    # Discrete-like numerics: integer columns with low cardinality behave as
    # categoricals for association purposes (e.g. num_children, rating, count).
    # Including them here lets both Cramér's V and the pb loop capture their
    # joint structure with true categoricals and other numeric columns.
    discrete_like_numerics = [
        c for c in numeric_cols
        if columns.numeric.get(c) is not None
        and columns.numeric[c].is_integer is True
        and columns.numeric[c].unique_count is not None
        and 1 < int(columns.numeric[c].unique_count) <= low_cardinality_threshold
    ]
    cat_cols_for_assoc = cat_cols_for_assoc + discrete_like_numerics
...
def _sanitize_dataframe(df: any) -> any:
    if df is None:
        return df
    for col in df.columns:
        if hasattr(df[col], 'dtype') and (df[col].dtype == object or df[col].dtype == 'O'):
...
def _load_dataframe(kind: str, source: str, *, max_rows: Optional[int]) -> Tuple[any, List[str]]:
    warnings: List[str] = []
    try:
        if kind in ("csv", "tsv"):  # G1: handle TSV with correct separator
            sep = "\t" if kind == "tsv" else ","
            df = pd.read_csv(source, sep=sep)
            if max_rows is not None:
                df = df.head(max_rows)
            return _sanitize_dataframe(df), warnings
        ...
        raise ValueError(f"Error loading {kind} dataset from {source}: {e}") from e
...
def _is_numeric_dtype(s: any) -> bool:
    ...
    if str(dt).startswith("int"):
        # Guard applies to integer dtypes.
        # High-cardinality integer columns are almost certainly IDs, not
        # true numeric features — classifying them as numeric causes
        # _check_no_nan to fail when the generator emits UUID strings for
        # them (bug: _determine_id_strategy falls back to "uuid4" for
        # non-monotonic integer IDs, producing strings that coerce to NaN).
...
def _main(argv: Optional[List[str]] = None) -> int:
    ...
    p.add_argument("--kind", choices=["csv", "tsv", "excel", "json", "parquet"], default=None)
    ...
    try:
        ...
        _sys.stdout.write(baseline.to_json(indent=2) + "\n")
        return 0
    except Exception as exc:
        _sys.stderr.write(f"Failed to build baseline: {str(exc)}\n")
        return 1
...
def _infer_kind_from_path(path: str) -> str:
    ...
    if ext == ".csv":
        return "csv"
    if ext == ".tsv":  # G1: TSV must be distinguished so the correct sep is used
        return "tsv"
