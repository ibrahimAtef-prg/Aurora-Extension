...
function addAgentMessage(text, artifact){
  ...
  if(artifact){
    ...
    if(artifact.type==='csv' && artifact.previewOnly){
      ...
      icon.textContent='📊'; lbl.textContent='Dataset ready'; btn.textContent=expLabel;
      ...
      chartWrap.style.cssText='background:var(--vscode-editor-background,#1e1e1e);border-radius:8px;padding:12px;margin-top:8px;width:480px;max-width:100%;overflow:hidden;box-sizing:border-box;';
      ...
      var canvasH=Math.max(140, Math.min(colCount*22+40, 340));
      canvas.style.cssText='display:block;width:456px;height:'+canvasH+'px;';
    ...
    } else if(artifact.type==='baseline_chart'){
      // ── Baseline-only column means chart ──────────────────────────────────
      var bChartWrap=document.createElement('div');
      bChartWrap.style.cssText='background:var(--vscode-editor-background,#1e1e1e);border-radius:8px;padding:12px;margin-top:8px;width:480px;max-width:100%;overflow:hidden;box-sizing:border-box;';
      var bChartTitle=document.createElement('div');
      bChartTitle.style.cssText='font-size:12px;font-weight:600;color:var(--vscode-foreground,#ccc);margin-bottom:2px;';
      bChartTitle.textContent='Baseline Distribution — Column Means (normalised)';
      var bChartSub=document.createElement('div');
      bChartSub.style.cssText='font-size:10px;color:#888;margin-bottom:8px;';
      bChartSub.textContent=(artifact.row_count ? artifact.row_count.toLocaleString()+' rows · ' : '')+'scale-normalised to [0,1]';
      bChartWrap.appendChild(bChartTitle);
      bChartWrap.appendChild(bChartSub);
      var bCanvas=document.createElement('canvas');
      var bColCount=(artifact.columns||[]).length;
      var bCanvasH=Math.max(120, Math.min(bColCount*22+40, 320));
      bCanvas.style.cssText='display:block;width:456px;height:'+bCanvasH+'px;';
      bCanvas.width=456; bCanvas.height=bCanvasH;
      bChartWrap.appendChild(bCanvas);
      el.appendChild(bChartWrap);
      setTimeout(function(){
        if(typeof Chart==='undefined'){ bChartTitle.textContent+=' (Chart.js unavailable)'; return; }
        var bCols=artifact.columns||[];
        var bNorms=artifact.norm_means||[];
        var bMeans=artifact.means||[];
        var bStds=artifact.stds||[];
        new Chart(bCanvas,{
          type:'bar',
          data:{
            labels:bCols,
            datasets:[{
              label:'Mean (normalised)',
              data:bNorms,
          ...
          options:{
            ...
            plugins:{
              ...
              tooltip:{callbacks:{label:function(ctx){
                var i=ctx.dataIndex;
                return ['Mean: '+( bMeans[i]!=null?bMeans[i]:'—'),'Std: '+(bStds[i]!=null?bStds[i]:'—')];
              }}}
            },
            scales:{
              x:{min:0,max:1,title:{display:true,text:'Normalised mean',color:'#888',font:{size:10}},ticks:{color:'var(--vscode-foreground,#ccc)',font:{size:9}},grid:{color:'rgba(255,255,255,0.06)'}},
              y:{ticks:{color:'var(--vscode-foreground,#ccc)',font:{size:9}},grid:{color:'rgba(255,255,255,0.03)'}},
            },
          },
        });
      ...
      // FIX-VISUAL: if the report artifact carries an embedded baseline_chart, render it inline
      if(isStatSummary && artifact.chart && artifact.chart.type==='baseline_chart'){
        var embChart=artifact.chart;
        var ecWrap=document.createElement('div');
        ecWrap.style.cssText='background:var(--vscode-editor-background,#1e1e1e);border-radius:8px;padding:12px;margin-top:8px;width:480px;max-width:100%;overflow:hidden;box-sizing:border-box;';
        var ecTitle=document.createElement('div');
        ecTitle.style.cssText='font-size:12px;font-weight:600;color:var(--vscode-foreground,#ccc);margin-bottom:2px;';
        ecTitle.textContent='Baseline Distribution — Column Means (normalised)';
        var ecSub=document.createElement('div');
        ecSub.style.cssText='font-size:10px;color:#888;margin-bottom:8px;';
        ecSub.textContent=(embChart.row_count ? embChart.row_count.toLocaleString()+' rows · ' : '')+'scale-normalised to [0,1]';
        ecWrap.appendChild(ecTitle); ecWrap.appendChild(ecSub);
        var ecCanvas=document.createElement('canvas');
        var ecCols=(embChart.columns||[]).length;
        var ecH=Math.max(120,Math.min(ecCols*22+40,320));
        ecCanvas.style.cssText='display:block;width:456px;height:'+ecH+'px;';
        ecCanvas.width=456; ecCanvas.height=ecH;
        ecWrap.appendChild(ecCanvas);
        el.appendChild(ecWrap);
        (function(canvas,ac){
          setTimeout(function(){
            if(typeof Chart==='undefined'){ ecTitle.textContent+=' (Chart.js unavailable)'; return; }
            new Chart(canvas,{
              type:'bar',
              data:{labels:ac.columns||[],datasets:[{label:'Mean (normalised)',data:ac.norm_means||[],backgroundColor:'rgba(56,139,253,0.75)',borderColor:'rgb(56,139,253)',borderWidth:1}]},
              options:{indexAxis:'y',responsive:false,maintainAspectRatio:false,
                plugins:{legend:{display:false},tooltip:{callbacks:{label:function(ctx){var i=ctx.dataIndex;return['Mean: '+(ac.means[i]!=null?ac.means[i]:'—'),'Std: '+(ac.stds[i]!=null?ac.stds[i]:'—')];}} }},
                scales:{x:{min:0,max:1,title:{display:true,text:'Normalised mean',color:'#888',font:{size:10}},ticks:{color:'var(--vscode-foreground,#ccc)',font:{size:9}},grid:{color:'rgba(255,255,255,0.06)'}},y:{ticks:{color:'var(--vscode-foreground,#ccc)',font:{size:9}},grid:{color:'rgba(255,255,255,0.03)'}}},
              },
            });
          },50);
        })(ecCanvas,embChart);
...
function agentExportCSV(btn){
  ...
  if(!rows.length){
    if(btn){ btn.disabled=false; btn.textContent='Export Synthetic'; }
    addAgentMessage('⚠ No sample data available to export. Run the generator first.');
    return;
  }
  // Detect format from D.sourceKind
  var fmt=((typeof D!=='undefined'&&D.sourceKind)||'csv').toLowerCase();
  ...
  if(fmt==='json'){
    ...
    filename='generated_data.jsonl';
  } else if(fmt==='tsv'){
    var cols=Object.keys(rows[0]||{});
    content=[cols.join('\t')].concat(rows.map(function(r){
      return cols.map(function(c){ return r[c]!=null?String(r[c]):''; }).join('\t');
    })).join('\n');
    filename='generated_data.tsv';
  } else {
    fmt='csv';
    var cols=Object.keys(rows[0]||{});
    content=[cols.join(',')].concat(rows.map(function(r){
  ...
  if(btn){ btn.textContent='✓ Saved'; setTimeout(function(){ btn.disabled=false; btn.textContent='Export Synthetic'; },2000); }
}

function agentExportJSON(btn){
  var rows=[];
  try{
    var src=(typeof D!=='undefined')&&(D.result||D.generator);
    rows=(src&&src.samples)||[];
  ...
  if(!rows.length){
    ...
    addAgentMessage('⚠ No sample data available to export.');
    return;
  }
  var content=JSON.stringify(rows,null,2);
  vscode.postMessage({command:'exportCSV', csv:content, filename:'generated_data.json', format:'json'});
  if(btn){ btn.textContent='✓ Saved'; setTimeout(function(){ btn.disabled=false; btn.textContent='Export JSON'; },2000); }
}
