...
function agentExportCSV(btn){
  ...
  if(!rows.length){
    if(btn){ btn.disabled=false; btn.textContent='Export Dataset'; }
  ...
  if(fmt==='xlsx'||fmt==='excel'||fmt==='parquet') fmt='csv';
  ...
  if(btn){ btn.textContent='✓ Saved'; setTimeout(function(){ btn.disabled=false; btn.textContent='Export Dataset'; },2000); }
  else { addAgentMessage('✅ Dataset exported — **'+filename+'** saved to your workspace.'); }
}
