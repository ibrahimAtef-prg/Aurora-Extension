...
function renderSecurity(){
  // Re-render whenever called — data may have changed since last render
  ...
  // Mode-aware: in synthetic mode prefer leakage (synthetic privacy analysis);
  // in original mode prefer scanReport (original file scan).
  const _isOrig = typeof isOriginalMode==='function' ? isOriginalMode() : true;
  const _activeLeakage = _isOrig ? null : (D.leakage||null);

  // Guard: no data yet
  const scan=_isOrig ? (D.scanReport||{}) : {};
  const statsEl=document.getElementById('sec-stats');
  const findEl=document.getElementById('sec-findings');
  if(!D.scanReport && !_activeLeakage && !D.leakage){
    // Pre-generation: derive PII surface from parser data + baseline
    var hasParserPii = typeof _parserData!=='undefined' && _parserData && _parserData.columns;
    var hasBaseline5  = !!(D.profile||D.baseline) && !!((D.profile||D.baseline).columns);
    if(hasParserPii || hasBaseline5){
      var b5 = D.profile||D.baseline||{};
      var parserPiiCols = hasParserPii
        ? _parserData.columns.filter(function(c){return c.pii&&c.pii!=='none';})
        : [];
      var allBaseCols5 = hasBaseline5
        ? [...Object.values((b5.columns.numeric)||{}),...Object.values((b5.columns.categorical)||{})]
        : [];
      var avgNull5 = allBaseCols5.length
        ? allBaseCols5.reduce(function(s,c){return s+(c.null_ratio||0);},0)/allBaseCols5.length
        : 0;
      var totalCols5 = allBaseCols5.length || (hasParserPii ? _parserData.columns.length : 0);
      var baseRows5 = (b5.meta&&(b5.meta.row_count||b5.meta.row_count_estimate))||(hasParserPii&&_parserData.rows)||0;
      var estRisk5 = Math.min(100, Math.round((parserPiiCols.length/Math.max(totalCols5,1))*60 + avgNull5*30));
      var rsCol5 = estRisk5>50?C.orange:C.green;
      if(statsEl){
        statsEl.innerHTML=
          '<div class="sec-stat"><div class="sec-stat-v" style="color:'+C.orange+'">'+parserPiiCols.length+'</div><div class="sec-stat-l">PII Columns</div></div>'+
          '<div class="sec-stat"><div class="sec-stat-v" style="color:'+rsCol5+'">'+estRisk5+'/100</div><div class="sec-stat-l">Risk Estimate</div></div>'+
          '<div class="sec-stat"><div class="sec-stat-v" style="color:'+C.p5+'">'+totalCols5+'</div><div class="sec-stat-l">Total Columns</div></div>'+
          '<div class="sec-stat"><div class="sec-stat-v" style="color:'+C.fg2+'">'+baseRows5.toLocaleString()+'</div><div class="sec-stat-l">Orig Rows</div></div>'+
          '<div class="sec-stat"><div class="sec-stat-v" style="color:'+(avgNull5>0.1?C.orange:C.green)+'">'+(avgNull5*100).toFixed(1)+'%</div><div class="sec-stat-l">Null Rate</div></div>';
      }
      if(findEl){
        if(parserPiiCols.length){
          findEl.innerHTML='<div style="font-size:11px;color:var(--fg2);margin-bottom:8px">PII columns detected in baseline dataset:</div>'+
            parserPiiCols.map(function(col){
              var sevCol=col.pii==='high'?C.red:col.pii==='medium'?C.orange:C.p4;
              var sevLabel=(col.pii==='high'?'HIGH':col.pii==='medium'?'MEDIUM':'LOW');
              return '<div class="sec-finding">'+
                '<span class="sf-type" style="color:'+sevCol+'">PII</span>'+
                '<span class="sf-cat">'+esc(col.piiReason||col.pii+' risk')+' — <span style="color:var(--fg3)">'+esc(col.name)+'</span></span>'+
                '<span class="sf-sev" style="background:'+sevCol+'22;color:'+sevCol+'">'+sevLabel+'</span>'+
              '</div>';
            }).join('')+
            '<div style="font-size:9px;color:var(--fg3);margin-top:8px;padding-top:6px;border-top:1px solid rgba(255,255,255,.06)">Full scan runs automatically after synthesis</div>';
        } else {
          findEl.innerHTML='<div style="color:var(--green);font-size:12px;padding:8px">No PII detected in baseline dataset.</div>';
        }
      }
      renderAttackGauges();
      return;
    }
    if(statsEl) statsEl.innerHTML='<div style="color:var(--fg3);font-size:11px;padding:10px">Parse a dataset to view security analysis.</div>';
    if(findEl)  findEl.innerHTML='';
    renderAttackGauges();
    return;
  }

  // Synthetic mode: leakage is the primary security source for synthetic data
  if(!_isOrig && _activeLeakage){
    const l=_activeLeakage;
    const piiCols=(l.pii_columns||[]);
    const rsRaw=l.dataset_risk_score!=null?l.dataset_risk_score:0;
    const rsNorm=rsRaw<=1?Math.round(rsRaw*100):Math.round(rsRaw);
    const rsCol=rsNorm>70?C.red:rsNorm>30?C.orange:C.green;
    const ps=l.privacy_score!=null?Math.round(l.privacy_score*100):null;
    if(statsEl){
      statsEl.innerHTML=
        '<div class="sec-stat"><div class="sec-stat-v" style="color:'+C.orange+'">'+piiCols.length+'</div><div class="sec-stat-l">PII Cols (Synth)</div></div>'+
        '<div class="sec-stat"><div class="sec-stat-v" style="color:'+rsCol+'">'+rsNorm+'/100</div><div class="sec-stat-l">Risk Score</div></div>'+
        '<div class="sec-stat"><div class="sec-stat-v" style="color:'+C.p5+'">'+(l.risk_level||'—')+'</div><div class="sec-stat-l">Risk Level</div></div>'+
        (ps!=null?'<div class="sec-stat"><div class="sec-stat-v" style="color:'+(ps>=75?C.green:ps>=50?C.orange:C.red)+'">'+ps+'%</div><div class="sec-stat-l">Privacy Score</div></div>':'')+
        '<div class="sec-stat"><div class="sec-stat-v" style="color:'+C.fg2+'">'+(l.statistical_drift||'—')+'</div><div class="sec-stat-l">Drift</div></div>';
    }
    if(findEl){
      if(piiCols.length){
        findEl.innerHTML='<div style="font-size:11px;color:var(--fg2);margin-bottom:8px">PII columns detected in synthetic output:</div>'+
        ...
        findEl.innerHTML='<div style="color:var(--green);font-size:12px;padding:8px">✓ No PII detected in synthetic output.</div>';
      }
    }
    renderAttackGauges();
    return;
  ...
  // Original mode: use leakage only when scanReport is absent
  if(!D.scanReport && D.leakage && _isOrig){
    const l=D.leakage||{};
