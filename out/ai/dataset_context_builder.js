...
function formatContextForLLM(ctx) {
    ...
    if (ctx.column_stats.length > 0) {
        lines.push('### Per-Column Statistics');
        ctx.column_stats.forEach(cs => {
            if (cs.type === 'numeric') {
                lines.push(`  ${cs.name} [numeric]:` +
                    ` mean=${cs.mean?.toFixed(3) ?? 'N/A'}` +
                    ` std=${cs.std?.toFixed(3) ?? 'N/A'}` +
                    ` min=${cs.min?.toFixed(3) ?? 'N/A'}` +
                    ` max=${cs.max?.toFixed(3) ?? 'N/A'}` +
                    ` null_ratio=${((cs.null_ratio ?? 0) * 100).toFixed(1)}%` +
                    (cs.drift_score != null ? ` drift=${cs.drift_score.toFixed(4)}` : '') +
                    (cs.is_pii ? ' *PII*' : ''));
            }
            else {
                lines.push(`  ${cs.name} [categorical]:` +
                    ` top_values=[${cs.top_values?.slice(0, 5).join(', ') ?? 'N/A'}]` +
                    ` unique_ratio=${((cs.unique_ratio ?? 0) * 100).toFixed(1)}%` +
                    ` null_ratio=${((cs.null_ratio ?? 0) * 100).toFixed(1)}%` +
                    (cs.is_pii ? ' *PII*' : ''));
            }
        });
...
function getValidNumbers(sdc) {
    ...
    for (const cs of sdc.column_stats) {
        addNum(cs.mean);
        addNum(cs.std);
        addNum(cs.min);
        addNum(cs.max);
        if (cs.null_ratio != null) {
            addNum(cs.null_ratio);
        }
        if (cs.unique_ratio != null) {
            addNum(cs.unique_ratio);
        }
    }
    return valid;
}
