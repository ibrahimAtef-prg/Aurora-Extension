...
exports.OpenRouterClient = exports.AURORA_AGENT_TOOLS = void 0;
exports._buildComparisonChartArtifact = _buildComparisonChartArtifact;
exports._buildBaselineChartArtifact = _buildBaselineChartArtifact;
exports._buildStatReport = _buildStatReport;
...
// ─────────────────────────────────────────────────────────────────────────────
exports.AURORA_AGENT_TOOLS = [
    {
        type: 'function',
        function: {
            name: 'get_dataset_state',
            description: 'Return the full current dataset state: row count, column names and types, ' +
    ...
    },
    {
        ...
        function: {
            name: 'generate_rows',
            description: 'Generate NEW synthetic rows from the ORIGINAL dataset baseline. ' +
            ...
            parameters: {
                ...
                properties: {
                    ...
                    generator: {
                        ...
                        description: 'Generator algorithm. Omit to reuse the last algorithm used.',
                    },
                    only: {
                        type: 'boolean',
                        description: 'If true, return these rows as a standalone result WITHOUT merging into the dataset. ' +
    ...
    },
    {
        ...
        function: {
            name: 'edit_dataset',
            description: 'Edit rows or columns in the dataset (synthetic or original samples). ' +
    ...
    },
    {
        ...
        function: {
            ...
            description: 'Run a deep analysis of the current dataset. ' +
            ...
            parameters: {
                ...
                properties: {
                    focus: {
                        ...
                        description: '"full" (default) runs all sub-analyses. ' +
    ...
    },
    {
        ...
        function: {
            ...
            description: 'Generate a detailed analytical / governance report from pipeline metrics. ' +
            ...
            parameters: {
                ...
                properties: {
                    type: {
                        ...
                        description: '"full" (default) combines all sections. ' +
    ...
    },
    {
        ...
        function: {
            name: 'stat_summary',
            description: 'Generate a statistical summary report. ' +
            ...
            parameters: {
                ...
                properties: {
                    scope: {
                        ...
                        description: '"baseline" = original data only | ' +
    ...
    },
    {
        ...
        function: {
            name: 'export_csv',
            description: 'Export the current synthetic dataset to a CSV file on disk and ' +
            ...
            parameters: {
                ...
                properties: {
                    filename: {
                        type: 'string',
                        description: 'Target filename without directory. Defaults to "aurora_dataset.csv".',
                    },
                },
            },
        },
    },
    {
        ...
        function: {
            ...
            description: 'Combine the current synthetic dataset with the original baseline data into one unified dataset. ' +
...
class OpenRouterClient {
            ...
            const req = https.request(options, (res) => {
                ...
                res.on('end', () => {
                    try {
                        ...
                        console.log(`[Aurora] Live free models from OpenRouter: ${allFreeIds.length} found, ` +
    ...
    _sanitizeContent(raw) {
        ...
        return raw
            ...
            .filter(line => {
            const t = line.trim();
            if (/^(user|model)\s+safety\s*:/i.test(t)) {
                return false;
            }
            if (/^safety\s+categor/i.test(t)) {
                return false;
            }
            if (/^content\s+filter/i.test(t)) {
                return false;
            }
            if (/^trigger\s+categor/i.test(t)) {
                return false;
            }
            return true;
        })
    ...
    _smartFallback(msg, ctx) {
        ...
        if (isGreeting) {
            if (ctx.hasData) {
                ...
                if (ctx.rowCount != null)
                    bits.push(`${ctx.rowCount} rows`);
                if (ctx.privacyScore)
                    bits.push(`privacy score ${ctx.privacyScore}`);
                if (ctx.columns?.length)
                    bits.push(`${ctx.columns.length} columns`);
        ...
        const parts = [];
        if (ctx.rowCount != null)
            parts.push(`${ctx.rowCount} rows`);
        if (ctx.privacyScore)
            parts.push(`privacy score ${ctx.privacyScore}`);
        if (ctx.riskLevel)
            parts.push(`risk level ${ctx.riskLevel}`);
        return parts.length
    ...
    async chat(messages, model) {
        ...
        if (model) {
            if (this.provider === 'openrouter' && !model.endsWith(':free')) {
                console.error(`[Aurora] Blocked attempt to use paid model "${model}" on OpenRouter. ` +
                    `Only models with the :free suffix are permitted.`);
                return {
                    content: '',
                    model: model,
                    error: `Model "${model}" is not a free OpenRouter model. ` +
    ...
    _chatOnceWithTools(messages, selectedModel, tools) {
        ...
        return new Promise((resolve) => {
            ...
            if (estimatedTokens > MAX_CONTEXT * 0.8) {
                console.warn(`[Aurora] Prompt payload is ~${estimatedTokens} tokens ` +
     ...
     */
    async _agenticLoop(messages, tools, toolExecutor, selectedModel, onToolProgress) {
        ...
        for (let i = 0; i < MAX_ITERATIONS; i++) {
            ...
            if (resp.error || !resp.message) {
                ...
                return this._agenticLoopTextFallback(messages, tools, toolExecutor, selectedModel, onToolProgress);
            }
     ...
     */
    async _agenticLoopTextFallback(messages, tools, toolExecutor, selectedModel, onToolProgress) {
        const MAX_ITERS = 8;
        ...
        const toolCatalogue = tools.map(t => {
            ...
            const paramLines = Object.entries(props)
                .map(([k, v]) => `    ${k} (${v.type ?? 'any'}): ${v.description ?? ''}`.trimEnd())
                .join('\n');
            return [
                `Tool: ${fn.name}`,
                `  ${fn.description}`,
        ...
        const baseSys = messages[0]?.content || '';
        const augmentedSys = {
            role: 'system',
            content: baseSys + [
                '',
        ...
        for (let i = 0; i < MAX_ITERS; i++) {
            ...
            if (inputKeyIdx !== -1) {
                ...
                if (jsonStart !== -1) {
                    ...
                    if (jsonEnd !== -1) {
                        try {
                            toolArgs = JSON.parse(after.slice(jsonStart, jsonEnd + 1));
                        }
                        catch { /* malformed */ }
                    }
                }
            ...
            // ─ Execute tool ─────────────────────────────────────────────────────
            if (onToolProgress) {
                onToolProgress(toolName, toolArgs, i);
            }
            let observation;
            try {
    ...
    buildSystemPrompt(ctx) {
        ...
        if (!ctx.datasetCtx) {
            ctx.datasetCtx = (0, dataset_context_builder_1.buildDatasetContext)(ctx);
        }
    ...
    buildLightResponse(ctx) {
        if (ctx.hasData) {
            ...
            const facts = [];
            if (ctx.rowCount != null)
                facts.push(`Your dataset has ${ctx.rowCount} rows`);
            if (ctx.privacyScore)
                facts.push(`a privacy score of ${ctx.privacyScore}`);
            if (ctx.riskLevel)
                facts.push(`risk level ${ctx.riskLevel}`);
...
function _formatAnalysisResult(raw) {
    try {
        ...
        lines.push(`**Baseline:** ${baselineRows ?? 'N/A'} rows` +
        ...
        if (bStats && sStats && Object.keys(bStats).length > 0) {
            ...
            lines.push('|--------|:------------:|:--------------:|:--:|:-----------:|:-------------:|');
            for (const col of (r.numeric_cols ?? [])) {
                const b = bStats[col];
                const s = sStats[col];
                if (!b || !s) {
                    continue;
            ...
            lines.push('|--------|------|-----|-----|-----|');
            for (const [col, s] of Object.entries(r.statistical)) {
        ...
        if (r.drift) {
            ...
            if (r.drift.drift_scores && typeof r.drift.drift_scores === 'object') {
                const top = Object.entries(r.drift.drift_scores)
                    .sort(([, a], [, b]) => b - a).slice(0, 5);
                if (top.length > 0) {
                    lines.push('- **Top drift columns:** ' + top.map(([c, v]) => `${c} (${v.toFixed(3)})`).join(', '));
                }
            }
        }
        return lines.join('\n');
...
function _buildComparisonChartArtifact(raw) {
    try {
        const r = JSON.parse(raw);
        const bStats = r.baseline?.statistical;
        const sStats = r.synthetic?.statistical;
        const cols = r.numeric_cols;
        if (!bStats || !sStats || !cols || cols.length === 0) {
            return null;
        }
        return {
...
// ─────────────────────────────────────────────────────────────────────────────
function _buildBaselineChartArtifact(baseline) {
    try {
...
// ─────────────────────────────────────────────────────────────────────────────
function _buildStatReport(ctx, scope = 'comparison') {
    const baselineCols = ctx.baseline?.columns?.numeric ?? {};
    const baselineRows = ctx.baseline?.meta?.row_count ?? null;
    const samples = ctx.result?.samples ?? [];
    const syntheticRows = samples.length > 0
        ? samples.length : (ctx.result?.row_count ?? null);
    const numericColNames = Object.keys(baselineCols);
    const leakage = ctx.leakage ?? {};
    ...
    if (scope === 'baseline') {
        ...
        for (const col of numericColNames) {
            ...
            if (flag) {
                alerts.push(`${col} (${deltaRaw.toFixed(1)}% shift)`);
            }
            lines.push(`| ${col} | ${bMean ?? '—'} | ${sMean ?? 'N/A'} | ${deltaStr}${flag}` +
                ` | ${bStd ?? '—'} | ${sStd ?? 'N/A'}` +
    ...
    if (scope === 'comparison' && leakage.drift_scores) {
        ...
        lines.push('');
        const driftEntries = Object.entries(leakage.drift_scores)
            .sort(([, a], [, b]) => b - a);
    ...
    if (leakage.detected_threats?.length > 0) {
        ...
        lines.push('');
        for (const t of leakage.detected_threats) {
...
function _formatExportResult(raw) {
    try {
        const r = JSON.parse(raw);
        if (r.error) {
            return `⚠️ Export failed: ${r.error}`;
        }
        if (r.success) {
...
function _formatReportResult(raw) {
    try {
        const r = JSON.parse(raw);
        if (r.error) {
            return `⚠️ Report generation failed: ${r.error}`;
        }
        if (r.success) {
            return [
...
// ─────────────────────────────────────────────────────────────────────────────
OpenRouterClient.prototype.agentChat = async function (history, newMessage, ctx, toolExecutor, onToolProgress) {
    const dsCtx = ctx.datasetCtx ?? (0, dataset_context_builder_1.buildDatasetContext)(ctx);
    const sdc = (0, dataset_context_builder_1.buildStructuredDatasetContext)(dsCtx);
    ...
    if (hasData) {
        ...
        if (riskLabel)
            ctxLines.push(`Risk level: ${riskLabel}`);
        if (sdc.dataset_risk_score != null)
            ctxLines.push(`Risk score: ${sdc.dataset_risk_score.toFixed(1)}/100`);
    ...
    const intent = this.detectIntent(newMessage);
    if (intent === 'greeting') {
    ...
    if (toolExecutor) {
        ...
        const agenticResult = await this['_agenticLoop'](messages, exports.AURORA_AGENT_TOOLS, toolExecutor, selectedModel, onToolProgress);
        ...
        if (isDegenerate && !agenticResult.toolsRun && toolExecutor) {
            ...
            if (/\banalyze\b|\banalysis\b/i.test(q)) {
                if (onToolProgress) {
                    onToolProgress('analyze_dataset', {}, 0);
                }
                ...
                try {
                    ...
                    if (parsed.success) {
                        agenticResult.artifact = { type: 'csv', filePath: parsed.file_path };
                    }
                }
                catch { /* non-critical */ }
            }
                ...
                if (onToolProgress) {
                    onToolProgress('get_dataset_state', {}, 0);
                }
                await toolExecutor('get_dataset_state', {});
                if (onToolProgress) {
                    onToolProgress('generate_rows', { count }, 1);
                }
                await toolExecutor('generate_rows', { count });
                if (onToolProgress) {
                    onToolProgress('merge_and_update', {}, 2);
                }
            ...
            else if (/\bstat(istical)?\s+(summ\w*|report)/i.test(q) ||
                ...
                if (onToolProgress) {
                    onToolProgress('stat_summary', { scope }, 0);
                }
                ...
                try {
                    ...
                    agenticResult.content = `✅ Statistical summary generated.`;
                }
                agenticResult.toolsRun = true;
                isDegenerate = false;
                ...
                if (onToolProgress) {
                    onToolProgress('generate_report', {}, 0);
                }
                ...
                try {
                    const parsed = JSON.parse(result);
                    if (parsed.success) {
                        agenticResult.artifact = { type: 'report', filePath: parsed.file_path };
                    }
                }
                catch { /* non-critical */ }
            }
        ...
        if (isDegenerate) {
            if (agenticResult.toolsRun) {
                ...
                agenticResult.content = this['_smartFallback'](newMessage, datasetCtx);
            }
        }
        return agenticResult;
    }
    // Non-agentic: direct LLM call with full context.
    return this.enforcedChat(messages, datasetCtx);
...
// ─────────────────────────────────────────────────────────────────────────────
OpenRouterClient.prototype.agentStatSummary = async function (ctx, scope = 'comparison') {
    const dsCtx = ctx.datasetCtx ?? (0, dataset_context_builder_1.buildDatasetContext)(ctx);
    const sdc = (0, dataset_context_builder_1.buildStructuredDatasetContext)(dsCtx);
    const riskLabel = dsCtx.risk_metrics?.risk_level ?? null;
    const rowCount = sdc.rows ?? null;
    ...
    if (scope === 'baseline') {
        ...
        for (const col of numericColNames) {
            ...
            compLines.push(`| ${col} | ${bMean ?? '—'} | ${sMean ?? 'N/A'} | ${deltaStr}${flag}` +
    ...
    const leakage = ctx.leakage ?? dsCtx.leakage ?? {};
    compLines.push('');
    ...
    if (leakage.drift_scores && typeof leakage.drift_scores === 'object') {
        const topDrift = Object.entries(leakage.drift_scores)
            .sort(([, a], [, b]) => b - a).slice(0, 5);
        if (topDrift.length > 0) {
            compLines.push(`- Top drift cols: ${topDrift.map(([c, v]) => `${c} (${v.toFixed(3)})`).join(', ')}`);
        }
    }
    if (leakage.detected_threats?.length > 0) {
        compLines.push('- Threats detected:');
        leakage.detected_threats.forEach((t) => {
