...
function agentHandleResponse(content, model, error, artifact){
  ...
  if(error){
    ...
    // ── PART 3: Strip hidden <actions> block BEFORE the text reaches the DOM ──
    var filtered=extractAndFilterActions(content||'');
    var cleanContent=filtered.cleanText;
    var pendingActions=filtered.actions;

    // ── PART 10: Auto-clean any leaked JSON that bypassed the system prompt ──
    cleanContent=autoCleanLeakedActions(cleanContent);

    _agentHistory.push({role:'assistant',content:cleanContent||'No response.',ts:ts,model:model});
    var tag=document.getElementById('agent-model-tag'); if(tag) tag.textContent=model||'';
    addAgentMessage(cleanContent||'No response.', artifact||null);

    // ── PART 4: Route extracted actions to the approval overlay ─────────────
    // NEVER execute immediately — always require explicit user confirmation.
    if(pendingActions.length>0){
      for(var _i=0;_i<pendingActions.length;_i++){
        var _act=pendingActions[_i];
        if(_act.action==='modify_generation'){
          agentShowControlConfirm(_act);
          break;
        }
      }
      console.log('[Aurora] PART 4: Pending actions held for user approval:',pendingActions);
    }
  }
...
// ── PART 3: UI Filter Layer ─────────────────────────────────────────────────
// Extracts the hidden <actions> channel injected by Aurora at the end of
// responses and REMOVES the tag from the text before it ever reaches the DOM.
// PART 5: This replaces agentParseActionBlocks which read legacy json-fence blocks.
function extractAndFilterActions(rawText){
  var actions=[];
  var cleanText=rawText;
  var actionsRx=/<actions>([\s\S]*?)<\/actions>/g;
  var m;
  while((m=actionsRx.exec(rawText))!==null){
    try{
      var parsed=JSON.parse(m[1].trim());
      if(Array.isArray(parsed)){ actions=actions.concat(parsed); }
      else if(parsed&&typeof parsed==='object'){ actions.push(parsed); }
    }catch(e){ console.warn('[Aurora] Could not parse <actions>:',m[1]); }
  }
...
// Belt-and-suspenders: if the model emitted a visible json-fence action block or
// a bare inline action object despite the system-prompt rules, strip it now.
function autoCleanLeakedActions(text){
  var BT=String.fromCharCode(96);
  var marker=BT+BT+BT;
  var cleaned=text;
  // Strip json-fence blocks whose body contains an "action" key
  var fencePattern=new RegExp(marker+'json\\s*\\{[^'+BT+']*"action"\\s*:[^'+BT+']*\\}\\s*'+marker,'gi');
  if(fencePattern.test(cleaned)){
    console.warn('[Aurora] PART 10: Auto-cleaned leaked json-fence action block.');
    cleaned=cleaned.replace(fencePattern,'').trim();
  }
  // Strip bare inline action objects
  var inlinePattern=/\{"action"\s*:\s*"[^"]*"[^}]*\}/g;
  if(inlinePattern.test(cleaned)){
    console.warn('[Aurora] PART 10: Auto-cleaned leaked inline action object.');
    cleaned=cleaned.replace(inlinePattern,'').trim();
  }
  return cleaned;
}
...
`;
//# sourceMappingURL=ui_agent.js.map