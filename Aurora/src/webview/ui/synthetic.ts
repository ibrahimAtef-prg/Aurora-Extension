...
function initDistCols(){
  ...
  // Include both numeric and categorical columns so the selector works for
  // datasets that have no numeric columns (categorical-only or mixed).
  var numCols=Object.keys((b.columns&&b.columns.numeric)||{});
  var catCols=Object.keys((b.columns&&b.columns.categorical)||{});
  _distCols=numCols.concat(catCols);
...
function renderDistComparison(col){
  ...
  var cat=(b.columns&&b.columns.categorical)||{};
  var samples=r.samples||_getSamples();
  var allCols=Object.keys(num).concat(Object.keys(cat));
  const dcSub=document.getElementById('dc-sub');
  if(!allCols.length||!samples.length){
    if(dcSub) dcSub.textContent='Not enough data';
    return;
  }
  const c=col||allCols[0];

  // ── Categorical column: frequency bar chart ───────────────────────
  if(cat[c]){
    const spec=cat[c];
    const origRatios=spec.top_value_ratios||{};
    const cats=Object.keys(origRatios).slice(0,12);
    if(!cats.length){if(dcSub) dcSub.textContent=c+' · no frequency data'; return;}
    // Synthetic frequencies from samples
    const synthTotal=Math.max(samples.length,1);
    const synthCounts={};
    samples.forEach(function(row){
      const v=String(row[c]??'');
      synthCounts[v]=(synthCounts[v]||0)+1;
    });
    const origData=cats.map(function(k){return Math.round((origRatios[k]||0)*100);});
    const synthData=cats.map(function(k){return Math.round(((synthCounts[k]||0)/synthTotal)*100);});
    const shortLabels=cats.map(function(k){return k.length>10?k.slice(0,10)+'…':k;});
    if(dcSub) dcSub.textContent=c+' · '+samples.length+' synthetic rows · categorical';
    getOrCreateChart('chart-dist',{
      type:'bar',
      data:{
        labels:shortLabels,
        datasets:[
          {label:'Original %',data:origData,backgroundColor:'transparent',borderColor:C.p4,borderWidth:1.5,type:'line',fill:false,pointRadius:3,tension:.3},
          {label:'Synthetic %',data:synthData,backgroundColor:'rgba(139,92,246,.25)',borderColor:C.p6,borderWidth:1,borderRadius:2}
        ]
      },
      options:{plugins:{legend:{display:false}},
        scales:{
          x:{ticks:{color:C.fg3,font:{size:8},maxRotation:30}},
          y:{grid:{color:'rgba(255,255,255,.04)'},ticks:{color:C.fg3,font:{size:8},callback:function(v){return v+'%';}}}
        },
  ...
  // ── Numeric column: quantile-based histogram ───────────────────────
