...
export function scanPrompt(
    ...
    const nonOverlapping: PromptFinding[] = [];
    let lastEnd = -1;
    ...
    const riskLevel: PromptScanResult['riskLevel'] =
        hasCritical ? 'dangerous' : hasHigh || nonOverlapping.length > 0 ? 'warning' : 'safe';
    ...
    if (nonOverlapping.length > 0) {
        ...
        // Policy evaluation
        const decision = evaluatePrompt(hasCritical, hasHigh, nonOverlapping.length, extensionPath);
