...
class CheckPoint:
    ...
    def __init__(
        ...
        self._frozen: bool        = False   # set True by freeze() after seal()
    ...
    def reset(self) -> None:
        ...
        if self._frozen:
            raise RuntimeError(
                "CheckPoint.reset() called after freeze() — "
                "checkpoint is immutable after freezing."
            )
        self._write({
    ...
    def commit(
        ...
        Raises
        ------
        RuntimeError if called after freeze().
        Raises
        ------
        ...
        if self._frozen:
            raise RuntimeError(
                "CheckPoint.commit() called after freeze() — "
                "checkpoint is immutable. No further rows can be added."
            )
    ...
    def seal(
        ...
        if self._frozen:
            raise RuntimeError(
                "CheckPoint.seal() called after freeze() — "
                "checkpoint is already immutable."
            )
        data = self._read_raw(self._path)
        data["status"]         = status
        data["updated_at"]     = _now_iso()
    ...
    def freeze(self) -> None:
        """
        Mark this checkpoint as immutable.

        Must be called immediately after seal() and before any validation
        reads. Once frozen:
          - commit() raises RuntimeError
          - reset() raises RuntimeError
          - seal() raises RuntimeError
          - peek() is allowed (read-only)
          - export() is allowed (read-only)

        Purpose
        -------
        Guarantees that the data read by peek() for validation is
        IDENTICAL to the data returned by export_guarded() — no write
        can occur between these two reads.
        """
        self._frozen = True
    ...
    def content_hash(self) -> str:
        """
        SHA-256 hash of the committed row data.

        Used to assert bit-identical consistency between validation input
        and exported output. Computed from the canonical JSON serialisation
        of the rows list (sorted keys, no whitespace).
        ...
        Example
        -------
        >>> h1 = cp.content_hash()   # after peek() for validation
        >>> records = export_guarded(cp, contract)
        >>> h2 = cp.content_hash()
        >>> assert h1 == h2, f"DATA DIVERGENCE: {h1} != {h2}"
        """
        rows = self._read_raw(self._path).get("rows", [])
        canonical = json.dumps(rows, ensure_ascii=True, sort_keys=True,
    ...
    def peek(self) -> List[Dict[str, Any]]:
        """
        Read committed rows FOR VALIDATION PURPOSES ONLY.

        MUST only be called after freeze(). This enforces that no further
        mutations can occur between this read and the final export_guarded()
        call, guaranteeing validated_data === returned_data.
        ...
        if not self._frozen:
            raise RuntimeError(
                "CheckPoint.peek() called before freeze(). "
                "Call cp.freeze() immediately after cp.seal() "
                "to lock the checkpoint before validation reads."
            )
        return self._read_raw(self._path).get("rows", [])
    ...
    def n_collected(self) -> int:
        """Current row count — always reads from disk to prevent multi-process staleness."""
        data = self._read_raw(self._path)
        return sum(c.get("n_rows", 0) for c in data.get("commits", []))
