...
def merge_outputs(
    ...
    if manifest.encoder is not None:
        decoded_df = manifest.encoder.decode(copula_output)
...
class CategoricalEncoder:
    ...
    def _fit_pb_encoding(
        ...
        # Guard: binary numeric partner produces means at exactly 0.0 / 1.0
        # which map to ±∞ under Φ⁻¹.  Fall through to cumprob instead.
        all_in_unit = all(0.0 <= v <= 1.0 for v in cat_map.values())
        has_boundary = any(v == 0.0 or v == 1.0 for v in cat_map.values())
        if all_in_unit and has_boundary:
            return None
